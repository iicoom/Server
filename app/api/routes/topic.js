...
export default (router) => {
  ...
  // 创建话题
    .post('/topic', async (ctx) => {
      const topicInfo = {
        ...
        topic_creator: ctx.checkBody('topic_creator').notEmpty('创建者不能为空！').trim().value,
        content: ctx.checkBody('content').notEmpty('内容不能为空！').trim().value,
        other_info: ctx.checkBody('other_info').empty().value,
      };
      ...
      ctx.body = await TopicService.create(topicInfo);
    })
  // 获取话题列表
    .get('/topic', async (ctx) => {
      const condition = {};
  ...
  // 获取话题详情
    .get('/topic/:id', needLogin, async (ctx) => {
  ...
  // 编辑话题
    .put('/topic/:id', needAdmin, async (ctx) => {
      const updateInfo = {
  ...
  // 删除话题
    .delete('/topic/:id', needAdmin, async (ctx) => {
      const anId = ctx.params.id;
